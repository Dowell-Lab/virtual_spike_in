...
## counts by 1 million
## FIXME Removed scaling factor because it breaks low counts like spike-ins
read -r PosScalingFactor <<< "$((PosCountsMapped))"
read -r NegScalingFactor <<< "$((NegCountsMapped))"

## We proceed to divide every count in our bedfile by the
## strand-specific scaling factor
awk	-F '\t' -v OFS='\t' -v norm="$PosScalingFactor" \
		'{print $1, $2, $3, $4, $5, $6, ($7*1000000) / norm}' "$PosStrandSums" > "$PosStrandMillionsNormalizedSums"
awk	-F '\t' -v OFS='\t' -v norm="$NegScalingFactor" \
		'{print $1, $2, $3, $4, $5, $6, ($7*1000000) / norm}' "$NegStrandSums" > "$NegStrandMillionsNormalizedSums"
